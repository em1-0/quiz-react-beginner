...
const Game = ({step, correct, onClickReact}) => {
  ...
  return (
    <div>
        ...
        <div className='flex flex-col sm:flex-row justify-between'>
            <p className='mb-3 text-xs font-semibold text-gray-400 '>Questions: <span className='font-normal text-white'>{step} out of {quests.length}</span></p>
      ...
      {/* Main part */}
      <h2 className='text-center px-3 py-3 bg-zinc-800 rounded-lg font-semibold mb-7 text-white text-xl'>{quests[step].title}</h2>
        <ul> 
            {quests[step].answers.map((item, index) => (
